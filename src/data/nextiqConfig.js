...
export const NEXTIQ_GUARDRAILS = [
  // ── Organization-level guardrails (always enforced) ──
  {
    id: 'GR-001',
  ...
  },
  {
    ...
    description: 'Never surface or reference data from one customer account in a conversation with a different customer.',
    severity: 'critical',
    status: 'active',
    level: 'organization',
  ...
  {
    id: 'GR-008',
    name: 'Conversation Tone Guardrail',
    description: 'Ensure all AI-generated responses maintain a professional, empathetic tone. Block sarcasm, blame, or dismissive language.',
    severity: 'warning',
    status: 'active',
    level: 'organization',
    condition: 'On every AI-generated REPLY or NBA text before delivery to the agent',
    constraint: 'Run tone analysis. Flag responses scoring below professional threshold. In Autopilot, rewrite the response. In assist mode, surface a warning badge on the draft.',
    applicableGoals: [],
  },
  // ── Goal-level guardrails (only when specific goal is active) ──
  {
    id: 'GR-002',
  ...
  {
    id: 'GR-009',
    name: 'Escalation Timeout for Security Goals',
    description: 'If an account security issue is unresolved after 10 minutes, auto-escalate to Tier 2 security team.',
    severity: 'warning',
    status: 'active',
    level: 'goal',
    condition: 'Goal is "Account Access & Security" AND conversation duration exceeds 10 minutes AND no resolution action has been completed',
    constraint: 'Surface a priority escalation NBA. In Autopilot, trigger auto-escalation to Tier 2. Log the timeout with full conversation context.',
